...
import pandas as pd
import plotly.express as px  # Modern visualizations
import seaborn as sns
import matplotlib.pyplot as plt

# Set page configuration
st.set_page_config(
    page_title="City Fire Prediction",
    page_icon="🌳",
    layout="wide",
...
# API Keys
OPENWEATHER_API_KEY = "0c18a1583614fbf74003bc19f6c89a72"  # Replace with your API key
FIRMS_API_KEY = "2402ceb75d735d32221a262c701f504e"  # Replace with your NASA FIRMS API key
...
if predict_button:
    with st.spinner("Fetching data and calculating risks..."):
        ...
        fire_count = fetch_fire_data(lat, lon)
        if not weather_data or fire_count is None:
            st.error("Failed to fetch required data")
            st.stop()

        # Calculate indices
        ffmc = calculate_ffmc(weather_data["temperature"], weather_data["humidity"], weather_data["wind_speed"], weather_data["rainfall"], None)
        dmc = calculate_dmc(weather_data["temperature"], weather_data["humidity"], weather_data["rainfall"], None)
        dc = calculate_dc(weather_data["temperature"], weather_data["rainfall"], None)
        ...
        try:
            ...
            # Display fire count
            st.write("### Fire Occurrences")
            st.metric("Fire Count", fire_count)
